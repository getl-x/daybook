...
一款私密、轻量、带固定问题引导的日记应用：**早上想想今天要做什么 → 白天随手记录 → 晚上总结**。用户永远不必面对空白编辑器。

### 1.2 第一迭代范围

- **用户名 + 密码**登录（注册关闭，账号由管理员侧添加，见 §6.2）；
- 今日页（由上到下三块）：突发事情（置顶、方便随手记）+ 早间记录（只有「今日计划」，点保存后收起）+ 晚间总结（到晚间提醒时间才出现）；
...
早上（默认 09:00 前后）收到提醒
        ↓
打开“今日” → 写「今日计划」→ 点「保存」→ 早间记录收成一行摘要
        └─ 今日计划  → 写入「今天」这一天的记录

白天任意时间 → 点今日页最上方的“记录此刻” → 新增一条带时间的突发事情

晚上（默认 21:00 前后）收到提醒 → 晚间总结到点自动出现在今日页（最早 18:00）→ 填写 → 自动保存
...
**一条记录描述"这一天自己"**；`day_events` / `day_meals` 写的是**这一天本身**，不是"替昨天留个位置"。填写入口是日历 / 单日详情（今日页早间记录只留「今日计划」）。日历、单日详情、搜索、统计、补写因此全部退化为"读一行"，不需要任何跨行映射。
...
| `day_events` | text | 这天发生了什么（在日历 / 单日详情里补写） |
...
完成状态一律**派生**，不存 `morning_completed_at`：今日页的"早间完成" = 今天的 `day_plan` 非空；"晚间完成" = 今天的 `evening_summary` 非空。今日页上没有"昨日回顾"，`day_events` / `day_meals` 只在日历 / 单日详情里补写，因此**不再参与**今日页进度（提醒的"仅未完成时提醒"用的是同一套派生规则）。
...
以"9 月 10 日的今日页"为例（今日页上每一块都只写"今天"那一行）：

| 界面位置 | 写入目标 | 字段 |
| --- | --- | --- |
| 早间记录（今日计划） | **9 月 10 日**的记录 | `day_plan` |
| 突发事情 | **9 月 10 日**的记录 | `entry_date = 9/10` |
| 晚间总结 | **9 月 10 日**的记录 | `evening_summary` |

1. 客户端**不传**"昨天"的日期；要给某一天补 `day_events` / `day_meals` 走日历 / 单日详情（D-1 由服务端算，避免客户端时区错误串天）；
2. 补写历史用 `PUT /v1/diaries/:date/review`，`:date` 是**被补写的那一天**；
...
2. 今日页从上到下三块：突发事情、早间记录（今日计划）、晚间总结；晚间总结在晚间提醒时间（最早 18:00）之前不出现。
3. 早间记录点「保存」后收成一行摘要（带「展开 / 修改」），今日计划落在 D 这条记录上；「早间已完成」只按今天的 `day_plan` 是否非空判定（API 断言）。
...
| `PUT /v1/diaries/:date/review` | 复用 `PATCH /v1/diaries/:date`（写那一天那一行） | 「回顾」是入口不是存储位置，无需单独接口 |
