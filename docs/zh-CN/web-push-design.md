...
- **D1 — 401/403 的处理（已决策）**：Node 版只把 404/410 当 `gone`，401/403 落进 `failed`。后果：VAPID 密钥一旦变化（换数据目录、重部署、换机器），那批订阅每天都失败却**永远不会被禁用**，日志天天刷。Go 版把 401/403 当作**永久失败并计入 `failure_count`**，复用 schema 已有的字段与禁用机制（阈值 `FAILURE_LIMIT = 10`，与 Node 相同）。选择「计入计数」而非「立即禁用」，是为了不误杀偶发 403（CDN 抖动）。
