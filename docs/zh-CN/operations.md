...
- 这三项（`ANDROID_KEYSTORE_PASSWORD` / `ANDROID_KEY_ALIAS` / `ANDROID_KEY_PASSWORD`）名字固定，同一套值可以给多个 App 复用。
...
**变量 + secret（推 Docker Hub 用）** —— 让 `docker-publish.yml` 把镜像**同时**推到 [Docker Hub](https://hub.docker.com/) 的 `docker.io/getl/daybook`（与 `ghcr.io/getl-x/daybook` 同一次构建、同名标签：`0.1.2` / `0.1` / `latest` / `sha-xxxxx`）。两处字段都在同一个 **Settings → Secrets and variables → Actions** 页面里，和上面 Android 签名那些并列：
...
生成 token：打开 https://hub.docker.com/settings/security → **New Access Token** → Description 随意（例：`daybook-ci-github-actions`）→ Access permissions 选 **Read & Write** → 生成后**只显示一次**，立即复制。
...
- `docker.io/getl/daybook` 这个仓库**不用手动建**：首次 push 会自动创建（**默认 public**）。
...
- 验证：`docker manifest inspect docker.io/getl/daybook:0.1.2`（public 包可匿名），或 `docker pull getl/daybook:0.1.2`。Docker Hub 对匿名拉取有频率限制（个人使用足够）；两个仓库内容完全一致。
